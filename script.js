...
const successSubmitMessageElement =
  document.getElementById("success-submit-msg");
...
GetMeeBeerApp.init = () => {
  ...
  GetMeeBeerApp.showHelperMessage(
    "Please select a state to see your list of Breweries"
  );
  ...
  resultsUlElement.addEventListener("click", (event) => {
    ...
    if (target.tagName === "BUTTON" || target.parentNode.tagName === "BUTTON") {
      const breweryCardId =
        target.dataset.breweryid || target.parentNode.dataset.breweryid;

      GetMeeBeerApp.addBreweryTofavouriteList(breweryCardId);
    }
  });
...
GetMeeBeerApp.createBreweryCard = (breweryData) => {
  ...
  <article id="${breweryData.id}" class="brewery-card">
    ...
    <div class="brewery-card__actions">
      <button data-breweryid="${
        breweryData.id
      }" class="brewery-card__actions__like-btn ${
    breweryData.liked ? "active" : ""
  }">
...
GetMeeBeerApp.addBreweryTofavouriteList = (id) => {
  const isPresent = GetMeeBeerApp.likedBreweries.some(
  ...
  if (!isPresent && favouriteBreweriesCount < 5) {
    const likedBrewery = GetMeeBeerApp.breweriesResults.filter(
      (brewery) => brewery.id === id
    )[0];
...
GetMeeBeerApp.removeBreweryFromfavouriteList = (id) => {
  const updatedfavouriteList = GetMeeBeerApp.likedBreweries.filter(
    (item) => item.id !== id
  );
...
GetMeeBeerApp.fetchBreweriesByState = (stateValue) => {
  ...
  fetch(
    `https://api.openbrewerydb.org/breweries?by_state=${encodeURI(stateValue)}`
  )
    ...
    .then((breweriesList) => {
      ...
      if (breweriesList.length === 0) {
        ...
        return GetMeeBeerApp.showHelperMessage(
          `Oops! looks like there are no breweries in: "${GetMeeBeerApp.selectedState}"`
        );
      }
      ...
      const allBreweriesCitiesNames = [
        ...new Set(breweriesList.map((item) => item.city)),
      ];
      ...
      selectCityElement.innerHTML =
        '<option selected disabled value="none">Filter by city</option>' +
        selectCityElement.innerHTML;
    })
    .catch(() => {
...
GetMeeBeerApp.filterBreweriesByCity = (city) => {
  const filteredList = GetMeeBeerApp.breweriesResults.filter(
    (brewery) => brewery.city === city
  );
...
GetMeeBeerApp.updateCityOptions = (options) => {
  selectCityElement.innerHTML = options
    .map((optionName) => `<option value="${optionName}">${optionName}</option>`)
    .join("");
};

GetMeeBeerApp.updateResultsList = (items) => {
  resultsUlElement.innerHTML = items
    .map((cardData) => GetMeeBeerApp.createBreweryCard(cardData))
    .join("");
};

GetMeeBeerApp.updatefavouriteList = (items) => {
  favouriteUlElement.innerHTML = items
    .map((cardData) => GetMeeBeerApp.createBreweryCard(cardData))
    .join("");
};
